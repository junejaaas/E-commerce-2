...
const updateProductById = async (productId, updateBody) => {
    ...
    if (mergedData.originalPrice) {
        ...
        } else if (mergedData.originalPrice && !updateBody.discountedPrice && !updateBody.discountPercentage) {
             // Re-sync if only originalPrice changed
             if (mergedData.discountPercentage) {
                 updateBody.discountedPrice = Math.round(mergedData.originalPrice * (1 - mergedData.discountPercentage / 100));
             } else if (mergedData.discountedPrice) {
                 updateBody.discountPercentage = Math.round(((mergedData.originalPrice - mergedData.discountedPrice) / mergedData.originalPrice) * 100);
             }
        }
    }
